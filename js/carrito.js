...
class Productos {
  constructor(id, cat, nombre, precio, img, marca, modelo, color, Retroiluminacion) {
    this.id = id;
    this.cat = cat
    this.nombre = nombre;
    this.precio = precio;
...
const buscarProductos = async () => {
  ...
  data.forEach((producto) => {
    stockProductos.push(new Productos(producto.id, producto.cat, producto.nombre, producto.precio, producto.img, producto.marca, producto.modelo, producto.color, producto.Retroiluminacion))
  });
  ...
  for (const producto of stockProductos) {
    ...
    card.innerHTML = `
             <figure class="card card-home">
                ...
                <p class="card-text">Retroiluminacion: ${producto.Retroiluminacion}</p>
...
sahumerios.addEventListener("click", () => {
  ...
  filtro = stockProductos.filter(producto => producto.cat === "teclados");
  filtrado(filtro)
});
defumadores.addEventListener("click", () => {
  contenedorProductos.innerHTML = "";
  filtro = stockProductos.filter(producto => producto.cat === "auriculares");
  filtrado(filtro)
});
velas.addEventListener("click", () => {
  contenedorProductos.innerHTML = "";
  filtro = stockProductos.filter(producto => producto.cat === "mouse");
  filtrado(filtro)
});
